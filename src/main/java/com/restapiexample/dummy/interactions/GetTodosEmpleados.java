...
public class GetTodosEmpleados implements Interaction {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        while (codigoRespuesta != 200) {
            try {
                respuesta = SerenityRest.given().baseUri(datos.leerExcel("Apis", "Data.xlsx", 5, 1))
        ...
        System.out.println("Los empleados registrados son: " + respuesta);
        String respuestaJson = SerenityRest.lastResponse().getBody().asString();
        EscribirEnJson.Json(respuestaJson,"resultados/GetTodosEmpleados.json");

    }

    public static Performable on(){
        return Instrumented.instanceOf(GetTodosEmpleados.class).withProperties();
    }
}
